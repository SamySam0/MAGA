...
def train(
    ...
    for epoch in range(1, n_epochs+1):
        ...
        if epoch % log_loss_per_n_epoch == 0 or epoch == n_epochs:
            ...
            print(f"Validity: {valid}, Unique: {unique}, Novel: {novel}", end='')
    ...
    print(f"Validity: {valid_s[best_epoch-1]}, Unique: {unique_s[best_epoch-1]}, Novel: {novel_s[best_epoch-1]}")
    ...
    with open(f'{checkpoint_path}/{checkpoint_name}_logs.txt', 'a') as f:
        ...
        for epoch in range(n_epochs):
            ...
            valid, unique, novel = valid_s[epoch], unique_s[epoch], novel_s[epoch]
            f.write(f"\nEpoch {epoch+1} ==> Training recon error: {train_recon_loss} | Validation recon error: {valid_recon_loss} | LR: {lr} | Validity: {valid} | Unique: {unique} | Novel: {novel}")

        # Best checkpoint infos
        f.write('\n\n===== Best checkpoint info =====')
        f.write(f"\nEpoch: {best_epoch}") 
        f.write(f"\nTRL: {train_recon_errors[best_epoch-1]}, VRL: {valid_recon_errors[best_epoch-1]}, LR: {lrs[best_epoch-1]} | ")
        f.write(f"Validity: {valid_s[best_epoch-1]}, Unique: {unique_s[best_epoch-1]}, Novel: {novel_s[best_epoch-1]}")
