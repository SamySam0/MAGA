...
class GANTrainer(object):
    ...
    def global_add_pool(x, batch, size=None):
        ...
        return torch.Tensor.scatter_('add', x, batch, dim=0, dim_size=size)
            ...
            for i, data in enumerate(data_loader):
                ...
                if ii % 100 == 0:
                    ...
                    if count_eval:
                        ...
                        fake_counts = []
                    if self.zinc or self.qm9:
                        print ('size x/ some distribution:', len(fake_data.batch)/self.batch_size, self.global_add_pool(torch.ones(len(fake_data.batch)).to(fake_data.batch.device), fake_data.batch))
                    if self.g_out_prob:
